...
Logos works the same way. At its foundation sits the **Logos Kernel**—a microkernel that handles the essential primitives every decentralised application needs. Above that, a networking layer provides peer discovery, connection management, and privacy-preserving communication through a mixnet. And above that sit **modules**—pluggable components for storage, messaging, blockchain, and whatever else developers need.

Logos ships with an opinionated default configuration—the kernel runtime, plus storage, messaging, and blockchain modules—that functions as Ubuntu does for Linux: a ready-made distribution that works out of the box. The **Logos App** is the launcher that starts this experience, but it's module-agnostic: it loads whatever module profile is configured, much like a desktop environment can run on any Linux distribution. Users choose which modules to run; developers can go further and assemble entirely different distributions with their own selection of modules and configurations.

## The Layers

The Logos tech stack is organised into distinct layers, each with a clear responsibility. From the bottom up:
...
In traditional operating systems, microkernels handle only the most fundamental operations: process management, memory allocation, and inter-process communication (IPC). Everything else—file systems, networking, device drivers—runs as a separate process in user space, communicating through the kernel's IPC mechanisms.
...
### Discovery, Peering, and Mixnet: The Networking Stack
...
But unlike TCP/IP, this layer has privacy built in from the ground up. The **mixnet** protects messages during their outbound phase, routing them through multiple relay nodes and mixing traffic patterns so that observers can't determine who is talking to whom. A [**capability discovery protocol**](https://lip.logos.co/ift-ts/raw/extended-kad-disco.html#api-specification) lets nodes advertise and find peers of interest for their applications without centralised registries, and the peering layer manages connections across the decentralised network.
...
**Storage** provides decentralised file storage and retrieval using content-addressed (CID-based) data. Need to host a frontend? Serve assets? Store user data without relying on corporate cloud providers? The [storage module](https://github.com/logos-storage/logos-storage-nim) handles it. Developers interact with it through a straightforward API: store a file, get back a CID; provide a CID, get back the file.

**Messaging** handles coordination—private, censorship-resistant communication between parties. It consists of two layers [Logos Delivery](https://github.com/logos-messaging/logos-delivery) provides the transport—publish-subscribe messaging for reliable delivery; [Logos Chat](https://github.com/logos-messaging/nim-chat-poc) uses Delivery as transport, providing encrypted 1:1 conversations and evolving toward group conversations. Both are being packaged as separate Logos Core modules—a [Delivery module](https://github.com/logos-co/logos-delivery-module) and a [Chat module](https://github.com/logos-co/logos-chatsdk-module). The Chat module currently embeds Delivery directly; future versions will use the standalone module through the kernel's IPC.

**Blockchain** provides decentralised compute and consensus. This isn't just another L1—it's a purpose-built blockchain using **Cryptarchia**, a private proof-of-stake consensus mechanism where validator identities and stake amounts remain hidden. The **Blend Network** provides network-level anonymity for block propagation. And the **Logos Execution Zone (LEZ)** enables programmable state—both public and private—where developers can deploy programs, run AMMs, transfer tokens, and create financial primitives with built-in privacy. The team recently published a [Proposer Privacy blog post](https://press.logos.co/article/why-proposer-anonymity) explaining why this matters.

**User Modules** are the wild card. Because Logos follows the microkernel pattern, anyone can build modules that plug into the same IPC infrastructure. The Logos Kernel loads them, manages their lifecycle, and enables them to communicate with other modules—whether they're Logos defaults or third-party additions. The new [`logos-module`](https://github.com/logos-co/logos-module) library and CLI tool abstract this interaction, and a [`logos-module-builder`](https://github.com/logos-co/logos-module-builder) simplifies the build process. SDKs exist for [C++](https://github.com/logos-co/logos-cpp-sdk), with Rust and other languages in development.
...
At the top of the stack sit the decentralised applications that people actually use. These compose the modules below them—a chat app uses the messaging module and the storage module; a DeFi application uses the blockchain module and the LEZ; a filesharing app uses the storage module.
...
This isn't theoretical architecture. The first testnet is coming together, and recent engineering reports give a concrete picture of what's being built.
...
**The networking layer is advancing.** The AnonComms team closed their DoS/Sybil protection deliverable with the publication of an [RLN DoS Protection LIP](https://lip.logos.co/ift-ts/raw/mix-spam-protection-rln.html) for libp2p mix. The [capability discovery protocol implementation](https://github.com/vacp2p/nim-libp2p/pull/2055) is in review. Mix message routing works in simulation, with optimisations including [concurrent proof generation and delay](https://github.com/vacp2p/nim-libp2p/pull/2069) underway. The demo chat app has been [enhanced with stats](https://github.com/logos-co/logos-chat-ui/pull/4) and mix improvements for [tag management](https://github.com/vacp2p/nim-libp2p/pull/2066) and [node pool handling](https://github.com/vacp2p/nim-libp2p/pull/2067).

**Storage is transitioning.** The codebase has been fully rebranded—[Docker images renamed](https://github.com/logos-storage/logos-storage-nim/pull/1387), [final Codex references removed](https://github.com/logos-storage/logos-storage-nim/issues/1396)—and the v0.3.0 release suspended marketplace components to focus on filesharing. An [improved Merkle tree spec](https://github.com/vacp2p/rfc-index/pull/281) has been published and is under review. Provider anonymity research is progressing through study of the [echomix pigeonhole storage protocol](https://hackmd.io/@codex-storage/Hyy8v2nrZg). A [simple C wrapper over libstorage](https://github.com/gmega/easylibstorage) with example [uploader](https://github.com/gmega/easylibstorage/blob/main/examples/uploader.c), [downloader](https://github.com/gmega/easylibstorage/blob/main/examples/downloader.c), and [console application](https://github.com/gmega/easylibstorage/blob/main/examples/storageconsole.c) makes integration more accessible. The module integrates with Logos Core, with [UI migrating from QWidgets to QML](https://github.com/logos-co/logos-storage-ui/pull/3).

**Messaging is taking shape.** The Logos Delivery [Send API](https://github.com/logos-messaging/logos-delivery/pull/3669) and [Health API](https://github.com/logos-messaging/logos-delivery/issues/3646) are complete, with the Receive/Subscribe API in progress. The [Logos Delivery module](https://github.com/logos-co/logos-delivery-module) for Logos Core is ready alongside a [Logos Chat module](https://github.com/logos-co/logos-chatsdk-module) and demo [Logos Chat UI module](https://github.com/logos-co/logos-chatsdk-ui). Logos Chat has working [double-ratchet encryption](https://github.com/logos-messaging/libchat/pull/32) with [FFI bindings](https://github.com/logos-messaging/libchat/pull/25), [conversation initialization](https://github.com/logos-messaging/libchat/pull/24), and [payload handling](https://github.com/logos-messaging/libchat/pull/44).
...
Testnet v0.1 is where this architecture becomes tangible—where you can run a node, store a file, send a message, transfer tokens, and push messages through a mixnet. Not on someone else's infrastructure. On yours.
