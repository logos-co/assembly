...
// Bundles `.inline.ts` files into browser-ready JS strings and compiles
// `.scss` to CSS strings, mirroring Quartz v5 core's own inline-script-loader
...
const inlineScriptPlugin: Plugin = {
  ...
  setup(parentBuild) {
    ...
    parentBuild.onLoad({ filter: /\.inline\.ts$/ }, async (args) => {
      ...
      // Inline scripts run in a <script> tag, not as ES modules — strip exports.
